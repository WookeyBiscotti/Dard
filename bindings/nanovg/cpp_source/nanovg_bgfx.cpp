...
BX_PRAGMA_DIAGNOSTIC_IGNORED_MSVC(4244); // warning C4244: '=' : conversion from
                                         // '' to '', possible loss of data

#include "fs_nanovg_fill.bin.h"
#include "vs_nanovg_fill.bin.h"

static const bgfx::EmbeddedShader s_embeddedShaders[] = {
    BGFX_EMBEDDED_SHADER(vs_nanovg_fill), BGFX_EMBEDDED_SHADER(fs_nanovg_fill),
...
static struct GLNVGtexture *glnvg__allocTexture(struct GLNVGcontext *gl) {
  ...
  if (tex == NULL) {
    if (gl->ntextures + 1 > gl->ctextures) {
      ...
      gl->textures = (struct GLNVGtexture *)BX_REALLOC(
          gl->allocator, gl->textures,
          sizeof(struct GLNVGtexture) * gl->ctextures);
      bx::memSet(&gl->textures[old], 0xff,
                 (gl->ctextures - old) * sizeof(struct GLNVGtexture));
...
static struct GLNVGtexture *glnvg__findTexture(struct GLNVGcontext *gl,
                                               int id) {
  int i;
  for (i = 0; i < gl->ntextures; i++) {
...
static int nvgRenderCreate(void *_userPtr) {
  ...
  gl->texMissing =
      bgfx::createTexture2D(4, 4, false, 1, bgfx::TextureFormat::BGRA8, 0, mem);

  gl->u_scissorMat =
      bgfx::createUniform("u_scissorMat", bgfx::UniformType::Mat3);
  ...
  gl->u_scissorExtScale =
      bgfx::createUniform("u_scissorExtScale", bgfx::UniformType::Vec4);
  gl->u_extentRadius =
      bgfx::createUniform("u_extentRadius", bgfx::UniformType::Vec4);
  ...
  if (bgfx::getRendererType() == bgfx::RendererType::Direct3D9) {
    gl->u_halfTexel =
        bgfx::createUniform("u_halfTexel", bgfx::UniformType::Vec4);
  } else {
  ...
  int align = 16;
  gl->fragSize = sizeof(struct GLNVGfragUniforms) + align -
                 sizeof(struct GLNVGfragUniforms) % align;

  return 1;
}

static int nvgRenderCreateTexture(void *_userPtr, int _type, int _width,
                                  int _height, int _flags,
  ...
  BX_ASSERT(tex->width >= 0 && tex->width <= bx::max<uint16_t>(),
            "Invalid tex width %d (max: %u)", tex->width, bx::max<uint16_t>());
  BX_ASSERT(tex->height >= 0 && tex->height <= bx::max<uint16_t>(),
            "Invalid tex height %d (max: %u)", tex->height,
            bx::max<uint16_t>());

  tex->id = bgfx::createTexture2D(
      uint16_t(tex->width), uint16_t(tex->height), false, 1,
      NVG_TEXTURE_RGBA == _type ? bgfx::TextureFormat::RGBA8
                                : bgfx::TextureFormat::R8,
      BGFX_SAMPLER_NONE);

  if (NULL != mem) {
    bgfx::updateTexture2D(tex->id, 0, 0, 0, 0, uint16_t(tex->width),
                          uint16_t(tex->height), mem);
  }
...
static int nvgRenderUpdateTexture(void *_userPtr, int image, int x, int y,
  ...
  BX_ASSERT(x >= 0 && x <= bx::max<uint16_t>(),
            "Invalid tex x pos %d (max: %u)", x, bx::max<uint16_t>());
  BX_ASSERT(y >= 0 && y <= bx::max<uint16_t>(),
            "Invalid tex y pos %d (max: %u)", y, bx::max<uint16_t>());
  BX_ASSERT(w >= 0 && w <= bx::max<uint16_t>(),
            "Invalid tex width %d (max: %u)", w, bx::max<uint16_t>());
  BX_ASSERT(h >= 0 && h <= bx::max<uint16_t>(),
            "Invalid tex width %d (max: %u)", h, bx::max<uint16_t>());

  bgfx::updateTexture2D(tex->id, 0, 0, uint16_t(x), uint16_t(y), uint16_t(w),
                        uint16_t(h), mem, UINT16_MAX);
...
static int nvgRenderGetTextureSize(void *_userPtr, int image, int *w, int *h) {
  ...
  if (NULL == tex || !bgfx::isValid(tex->id)) {
    return 0;
  }
...
static int glnvg__convertPaint(struct GLNVGcontext *gl,
...
static struct GLNVGfragUniforms *nvg__fragUniformPtr(struct GLNVGcontext *gl,
                                                     int i) {
  return (struct GLNVGfragUniforms *)&gl->uniforms[i];
}

static void nvgRenderSetUniforms(struct GLNVGcontext *gl, int uniformOffset,
                                 int image) {
...
static void nvgRenderViewport(void *_userPtr, float width, float height,
                              float devicePixelRatio) {
  struct GLNVGcontext *gl = (struct GLNVGcontext *)_userPtr;
  gl->view[0] = width;
  gl->view[1] = height;
  bgfx::setViewRect(gl->viewId, 0, 0, width * devicePixelRatio,
...
static void glnvg__fill(struct GLNVGcontext *gl, struct GLNVGcall *call) {
  ...
  for (i = 0; i < npaths; i++) {
    if (2 < paths[i].fillCount) {
      bgfx::setState(0);
      bgfx::setStencil(
          0 | BGFX_STENCIL_TEST_ALWAYS | BGFX_STENCIL_FUNC_RMASK(0xff) |
  ...
  if (gl->edgeAntiAlias) {
    ...
    for (i = 0; i < npaths; i++) {
      ...
      bgfx::setStencil(
          0 | BGFX_STENCIL_TEST_EQUAL | BGFX_STENCIL_FUNC_RMASK(0xff) |
          BGFX_STENCIL_OP_FAIL_S_KEEP | BGFX_STENCIL_OP_FAIL_Z_KEEP |
          BGFX_STENCIL_OP_PASS_Z_KEEP);
      bgfx::setVertexBuffer(0, &gl->tvb, paths[i].strokeOffset,
                            paths[i].strokeCount);
      bgfx::setTexture(0, gl->s_tex, gl->th);
      bgfx::submit(gl->viewId, gl->prog);
    }
  ...
  bgfx::setTexture(0, gl->s_tex, gl->th);
  bgfx::setStencil(0 | BGFX_STENCIL_TEST_NOTEQUAL |
                   BGFX_STENCIL_FUNC_RMASK(0xff) | BGFX_STENCIL_OP_FAIL_S_ZERO |
                   BGFX_STENCIL_OP_FAIL_Z_ZERO | BGFX_STENCIL_OP_PASS_Z_ZERO);
  bgfx::submit(gl->viewId, gl->prog);
}
...
glnvg__blendCompositeOperation(NVGcompositeOperationState op) {
  ...
  if (blend.srcRGB == BGFX_STATE_NONE || blend.dstRGB == BGFX_STATE_NONE ||
      blend.srcAlpha == BGFX_STATE_NONE || blend.dstAlpha == BGFX_STATE_NONE) {
...
static void nvgRenderFlush(void *_userPtr) {
  ...
  if (gl->ncalls > 0) {
    ...
    if (avail < gl->nverts) {
      ...
      BX_WARN(
          true,
          "Vertex number truncated due to transient vertex buffer overflow");
      if (gl->nverts < 2) {
        goto _cleanup;
      }
    }
    ...
    if (allocated < gl->nverts) {
      // this branch should never be taken as we've already checked the
      // transient vertex buffer size
      gl->nverts = allocated;
      BX_WARN(
          true,
          "Vertex number truncated due to transient vertex buffer overflow");
    ...
    for (uint32_t ii = 0, num = gl->ncalls; ii < num; ++ii) {
      ...
      gl->state =
          BGFX_STATE_BLEND_FUNC_SEPARATE(blend->srcRGB, blend->dstRGB,
...
static struct GLNVGcall *glnvg__allocCall(struct GLNVGcontext *gl) {
  ...
  if (gl->ncalls + 1 > gl->ccalls) {
    ...
    gl->calls = (struct GLNVGcall *)BX_REALLOC(
        gl->allocator, gl->calls, sizeof(struct GLNVGcall) * gl->ccalls);
  }
...
static int glnvg__allocPaths(struct GLNVGcontext *gl, int n) {
  ...
  if (gl->npaths + n > gl->cpaths) {
    ...
    paths = (GLNVGpath *)BX_REALLOC(gl->allocator, gl->paths,
...
static int glnvg__allocVerts(GLNVGcontext *gl, int n) {
  ...
  if (gl->nverts + n > gl->cverts) {
    ...
    verts = (NVGvertex *)BX_REALLOC(gl->allocator, gl->verts,
...
static int glnvg__allocFragUniforms(struct GLNVGcontext *gl, int n) {
  ...
  if (gl->nuniforms + n > gl->cuniforms) {
    ...
    gl->uniforms = (unsigned char *)BX_REALLOC(gl->allocator, gl->uniforms,
                                               gl->cuniforms * structSize);
  }
...
static void glnvg__vset(struct NVGvertex *vtx, float x, float y, float u,
                        float v) {
  vtx->x = x;
  vtx->y = y;
  vtx->u = u;
...
static void nvgRenderFill(void *_userPtr, NVGpaint *paint,
  ...
  for (i = 0; i < npaths; i++) {
    ...
    if (path->nfill > 0) {
      ...
      bx::memCopy(&gl->verts[offset], path->fill,
    ...
    if (path->nstroke > 0) {
      ...
      bx::memCopy(&gl->verts[offset], path->stroke,
...
static void nvgRenderStroke(void *_userPtr, struct NVGpaint *paint,
                            NVGcompositeOperationState compositeOperation,
  ...
  call->uniformOffset = glnvg__allocFragUniforms(gl, 1);
  glnvg__convertPaint(gl, nvg__fragUniformPtr(gl, call->uniformOffset), paint,
                      scissor, strokeWidth, fringe);
}

static void nvgRenderTriangles(void *_userPtr, struct NVGpaint *paint,
                               NVGcompositeOperationState compositeOperation,
  ...
  bx::memCopy(&gl->verts[call->vertexOffset], verts,
...
NVGcontext *nvgCreate(int32_t _edgeaa, bgfx::ViewId _viewId,
  ...
  struct GLNVGcontext *gl =
      (struct GLNVGcontext *)BX_ALLOC(_allocator, sizeof(struct GLNVGcontext));
  if (gl == NULL) {
    goto error;
  }
...
void nvgDelete(NVGcontext *_ctx) { nvgDeleteInternal(_ctx); }
...
NVGLUframebuffer *nvgluCreateFramebuffer(NVGcontext *ctx, int32_t width,
                                         int32_t height, int32_t imageFlags,
                                         bgfx::ViewId viewId) {
  NVGLUframebuffer *framebuffer =
      nvgluCreateFramebuffer(ctx, width, height, imageFlags);
...
NVGLUframebuffer *nvgluCreateFramebuffer(NVGcontext *_ctx, int32_t _width,
  ...
  BX_ASSERT(_width >= 0 && _width <= bx::max<uint16_t>(),
            "Invalid tex width %d (max: %u)", _width, bx::max<uint16_t>());
  BX_ASSERT(_height >= 0 && _height <= bx::max<uint16_t>(),
            "Invalid tex height %d (max: %u)", _height, bx::max<uint16_t>());
  const uint16_t w = uint16_t(_width);
  const uint16_t h = uint16_t(_height);
  bgfx::TextureHandle textures[] = {
      bgfx::createTexture2D(w, h, false, 1, bgfx::TextureFormat::RGBA8,
                            BGFX_TEXTURE_RT),
      bgfx::createTexture2D(w, h, false, 1, bgfx::TextureFormat::D24S8,
                            BGFX_TEXTURE_RT | BGFX_TEXTURE_RT_WRITE_ONLY)};
  bgfx::FrameBufferHandle fbh =
      bgfx::createFrameBuffer(BX_COUNTOF(textures), textures, true);
...
NVGLUframebuffer *nvgluCreateFramebuffer(NVGcontext *ctx, int32_t imageFlags,
                                         bgfx::ViewId viewId) {
...
NVGLUframebuffer *nvgluCreateFramebuffer(NVGcontext *_ctx,
                                         int32_t _imageFlags) {
  BX_UNUSED(_imageFlags);
  bgfx::TextureHandle textures[] = {
...
void nvgluSetViewFramebuffer(bgfx::ViewId _viewId,
...
int nvgCreateBgfxTexture(struct NVGcontext *_ctx, bgfx::TextureHandle _id,
...
NVGcontext *nvgCreateC(int _edgeaa, unsigned short _viewId) {
  return nvgCreate(_edgeaa, _viewId);
}

void nvgDeleteC(NVGcontext *_ctx) { nvgDelete(_ctx); }
