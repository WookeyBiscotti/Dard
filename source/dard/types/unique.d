 ...
 */
struct Unique(UniqueType, Allocator = typeof(theAllocator()),
...
private:
    ...
    void deleteObject() @safe {
        ...
        if (_object !is null)
            () @trusted { _allocator.dispose(_object); }();
        static if (is(Type == class)) {
            ...
            () @trusted { GC.removeRange(_object); }();
        }
    }
...
private template makeObject(Flag!"supportGC" supportGC, args...) {
    void makeObject(Type, A)(ref Unique!(Type, A) u) {
        ...
        static if (is(Type == class) || is(Type == interface)) {
            ...
            () @trusted { GC.addRange(u._object, Type.sizeof); }();
        }
    }
}
